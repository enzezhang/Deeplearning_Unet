...
def raster_copy( s_fh, s_xoff, s_yoff, s_xsize, s_ysize, s_band_n,
    ...
    w_old=w_band.ReadAsArray(t_xoff, t_yoff, t_xsize, t_ysize)
    w_add=np.ones([s_ysize, s_xsize])
    w_new=w_old+w_add
    data=(data_src+data_dst)
    t_band.WriteArray(data, t_xoff, t_yoff)
    w_band.WriteArray(w_new,t_xoff,t_yoff)
...
def raster_copy_with_nodata( s_fh, s_xoff, s_yoff, s_xsize, s_ysize, s_band_n,
    ...
    data_src = s_band.ReadAsArray( s_xoff, s_yoff, s_xsize, s_ysize,
                                   t_xsize, t_ysize )
    data_dst = t_band.ReadAsArray( t_xoff, t_yoff, t_xsize, t_ysize )
    data = (data_src / 2 + data_dst / 2)

    nodata_test = Numeric.equal(data_src,nodata)
    to_write = Numeric.choose( nodata_test, (data_src, data) )
...
def main( argv=None ):
    ...
    if t_fh is None:
        ...
        t_fh = Driver.Create( out_file, xsize, ysize, 2,
