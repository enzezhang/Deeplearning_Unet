...
parser.add_argument('--edge',default=0,type=int,help='switch to train edge, if this is on batchSize should be 1, 0 is off 1 is on')
...
dataset = RemoteSensingImg(args.dataroot, args.list, patch_w,patch_h, overlay_x,overlay_y,args.edge)
...
def train(epoch):
    ...
    for i, (x, y) in enumerate(train_loader):
        ...
        if x_shape[2] < crop_height or x_shape[3] < crop_width:
           ...
           y_true_expand=torch.zeros(x_shape[0], x_shape[1], crop_height, crop_width)
           y_true_expand[:, :, :x_shape[2], :x_shape[3]]=y

          # print(x.size())

           y_true=Variable(y_true_expand)
           x = Variable(x_expand)
